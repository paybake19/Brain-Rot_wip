...
function TaskRow({ title, agent, status, pct }: typeof TASKS[0]) {
  return (
    <div className="flex items-start gap-3 px-2.5 py-2.5 rounded-xl hover:bg-[rgb(var(--text)/0.03)] transition-colors duration-200 group cursor-default">
        ...
        {status === 'active' && (
          <div className="flex items-center gap-2 mt-2">
            <div className="h-.75 flex-1 bg-[rgb(var(--text)/0.06)] rounded-full overflow-hidden">
              <div
                className="h-full bg-[rgb(var(--accent)/0.6)] rounded-full transition-all duration-700"
                style={{ width: `${pct}%` }}
              />
            </div>
...
export default function RightPanel() {
  return (
    <aside className="w-[320px] shrink-0 flex flex-col border-l border-[rgb(var(--text)/0.04)] bg-[rgb(var(--panel)/0.6)] overflow-hidden transition-colors duration-700">
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-3 shrink-0">
        ...
        <button className="text-[10px] font-medium text-[rgb(var(--text)/0.3)] hover:text-[rgb(var(--text)/0.6)] transition-colors bg-[rgb(var(--text)/0.04)] px-2 py-1 rounded-md cursor-pointer">
          View All
        </button>
      </div>
      ...
      <div className="border-t border-[rgb(var(--text)/0.04)] shrink-0 mt-2">
        ...
        <div className="mx-3 mb-3 p-3 bg-[rgb(var(--text)/0.02)] border border-[rgb(var(--text)/0.05)] rounded-xl shadow-[inset_0_1px_0_rgb(var(--text)/0.02)] backdrop-blur-md transition-colors duration-700">

          {/* Vault name row */}
          <div className="flex items-center gap-3 mb-4">
            {/* Obsidian vault icon */}
            <div className="w-10 h-10 rounded-xl bg-[rgb(var(--accent)/0.08)] border border-[rgb(var(--accent)/0.15)] flex items-center justify-center shrink-0 transition-colors duration-700">
              <svg width="18" height="18" viewBox="0 0 17 17" fill="none">
                <path d="M8.5 1.5L2 5.5v6l6.5 4 6.5-4v-6L8.5 1.5z"
                  stroke="rgb(var(--accent))" strokeWidth="1.2" strokeLinejoin="round"/>
                <path d="M8.5 1.5v10M2 5.5l6.5 4 6.5-4"
                  stroke="rgb(var(--accent))" strokeWidth="1.1" strokeLinejoin="round"/>
              </svg>
            </div>
            ...
            <button className="text-[10px] font-medium text-[rgb(var(--text)/0.4)] hover:text-[rgb(var(--text))] transition-colors shrink-0 bg-[rgb(var(--text)/0.04)] px-2 py-1.5 rounded-lg border border-[rgb(var(--text)/0.04)] cursor-pointer">
              Change
            </button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 text-center mb-3">
            {VAULT_STATS.map(([l, v]) => (
              <div key={l} className="bg-[rgb(var(--bg)/0.5)] rounded-lg py-2 border border-[rgb(var(--text)/0.02)] transition-colors duration-700">
                <div className="text-[9px] text-[rgb(var(--text)/0.3)] mb-1">{l}</div>
            ...
            {VAULT_META.map(([l, v, c]) => (
              <div key={l}>
                ...
                <div
                  ...
                  style={{ color: c === 'accent' ? 'rgb(var(--accent))' : 'rgb(var(--text) / 0.6)' }}
                >
                  {v}
                </div>
              </div>
          ...
          <div className="flex gap-2">
            <button className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-[11px] font-medium border border-[rgb(var(--text)/0.08)] text-[rgb(var(--text)/0.6)] hover:text-[rgb(var(--text))] hover:bg-[rgb(var(--text)/0.04)] transition-all cursor-pointer">
              <svg width="12" height="12" viewBox="0 0 11 11" fill="none">
                <path d="M9.5 5.5a4 4 0 11-1.2-2.9"
            ...
            <button className="w-9 h-9 flex items-center justify-center rounded-lg border border-[rgb(var(--text)/0.08)] text-[rgb(var(--text)/0.4)] hover:text-[rgb(var(--text))] hover:bg-[rgb(var(--text)/0.04)] transition-all cursor-pointer">
